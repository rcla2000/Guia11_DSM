...
class Producto(context: Context?) {
    ...
    fun generarContentValues(
        ...
        valores.put(Producto.COL_IDCATEGORIA, idcategoria)
        valores.put(Producto.COL_DESCRIPCION, descripcion)
        valores.put(Producto.COL_PRECIO, precio)
        valores.put(Producto.COL_CANTIDAD, cantidad)
        return valores
    }
    ...
    fun searchProductosAll(): Cursor? {
        ...
        return db!!.query(
            TABLE_NAME_PRODUCTOS, columns,
            null, null, null, null, "${Producto.COL_DESCRIPCION} ASC"
        )
    }
}